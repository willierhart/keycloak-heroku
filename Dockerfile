...
RUN echo '#!/bin/sh' > /opt/keycloak/entrypoint.sh \
    && echo '/opt/keycloak/bin/kc.sh start --hostname-strict=false --http-port=$PORT --proxy=edge --db=postgres --db-url=$DB_URL --db-username=$DB_USERNAME --db-password=$DB_PASSWORD --spi-phone-default-service=dummy --spi-phone-default-duplicate-phone=false --optimized' >> /opt/keycloak/entrypoint.sh \
